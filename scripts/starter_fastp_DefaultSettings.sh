...
do
	...
	then
		...
		then  
			fastp -i $fastq -I $(echo $fastq | sed "s/$Fw$filetype$/$Rv$filetype/1") -o $PATH_output/$SampleName"_fastp"$Fw$filetype -O $PATH_output/$SampleName"_fastp"$Rv$filetype --thread $cpu --json $PATH_output/$SampleName.json --html $PATH_output/$SampleName.html --report_title $PATH_output/fastp_report --phred64
		else
			fastp -i $fastq -I $(echo $fastq | sed "s/$Fw$filetype$/$Rv$filetype/1") -o $PATH_output/$SampleName"_fastp"$Fw$filetype -O $PATH_output/$SampleName"_fastp"$Rv$filetype --thread $cpu --json $PATH_output/$SampleName.json --html $PATH_output/$SampleName.html --report_title $PATH_output/fastp_report
		fi
	fi
done
